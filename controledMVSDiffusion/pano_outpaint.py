...
class PanoOutpaintGenerator(pl.LightningModule):
    def __init__(self, config):
        ...
        self.tokenizer = CLIPTokenizer.from_pretrained(
            config['model']['model_id'], subfolder="tokenizer", torch_dtype=torch.float32)
        self.text_encoder = CLIPTextModel.from_pretrained(
            config['model']['model_id'], subfolder="text_encoder", torch_dtype=torch.float32)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # if device is cpu make it be cuda
        T = batch['R'][:, :, :3, 3:]
        R = batch['R'][:, :, :3, :3]
        K = batch['K'][:, :, :3, :3]
        batch["T"] = T
    ...
    def gen_cls_free_guide_pair(self, latents, timestep, prompt_embd, batch):
        latents = torch.cat([latents]*2) # one conditional, one unconditional
        timestep = torch.cat([timestep]*2) # one conditional, one unconditional
    ...
    @torch.no_grad()
    def save_image(self, images_pred, images,blur_images, prompt, batch_idx):
